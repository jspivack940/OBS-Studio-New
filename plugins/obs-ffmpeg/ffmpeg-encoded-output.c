...
struct obs_output_info ffmpeg_encoded_output_info = { .id =
		"ffmpeg_encoded_output", .flags = OBS_OUTPUT_AV |
OBS_OUTPUT_ENCODED | OBS_OUTPUT_SERVICE | OBS_OUTPUT_MULTI_TRACK,
		.encoded_video_codecs = "h264", .encoded_audio_codecs = "aac",
		.get_name = ffmpeg_encoded_output_getname, .create =
				ffmpeg_encoded_output_create, .destroy =
				ffmpeg_encoded_output_destroy, .start =
				ffmpeg_encoded_output_start, .stop =
				ffmpeg_encoded_output_stop, .encoded_packet =
...
static void *ffmpeg_encoded_output_create(obs_data_t *settings,
		obs_output_t *output)
{
	struct ffmpeg_encoded_output *stream = bzalloc(
			sizeof(struct ffmpeg_encoded_output));
	...
	fail: ffmpeg_encoded_output_destroy(stream);
	return NULL ;
}
		...
		struct encoder_packet *packet)
{
	...
	if (packet->type == OBS_ENCODER_VIDEO) {
		if (!stream->got_first_video) {
			stream->start_dts_offset = get_ms_time(packet,
					packet->dts);
...
static obs_properties_t *ffmpeg_encoded_output_properties(void *unused)
{
	...
	obs_properties_add_int(props, OPT_DROP_THRESHOLD,
			obs_module_text("FFmpegEncodedOutput.DropThreshold"),
			200, 10000, 100);
	obs_properties_add_bool(props, OPT_LOWLATENCY_ENABLED,
			obs_module_text("FFmpegEncodedOutput.LowLatencyMode"));
...
static void *send_thread(void *data)
{
	...
	stream->sent_video_header = false;
...
static bool send_video_header(struct ffmpeg_encoded_output *stream)
{
	...
	struct encoder_packet packet = { .type = OBS_ENCODER_VIDEO,
			.timebase_den = 1, .keyframe = true };

	stream->sent_video_header = true;
...
static bool init_connect(struct ffmpeg_encoded_output *stream)
{
	...
	drop_p = (int64_t) obs_data_get_int(settings,
			OPT_PFRAME_DROP_THRESHOLD);
	stream->max_shutdown_time_sec = (int) obs_data_get_int(settings,
			OPT_MAX_SHUTDOWN_TIME_SEC);
		...
		int highest_priority, bool pframes)
{
	...
	debug("Dropped %s, prev packet count: %d, new packet count: %d",
			name, start_packets, (int)num_buffered_packets(stream));
#endif
}
		...
		bool pframes)
{
	...
	int64_t drop_threshold =
			pframes ? stream->pframe_drop_threshold_usec :
...
static int proto_try_connect(struct ffmpeg_encoded_output *stream)
{
	...
	obs_encoder_t *aencoder = obs_output_get_audio_encoder(stream->output,
			0);
