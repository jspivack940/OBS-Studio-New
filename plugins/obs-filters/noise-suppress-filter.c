...
#ifdef LIBNVAFX_ENABLED
	// code from another branch, when trying to load the dll from the redist
	//char *sdk_path = nvafx_get_path();
	//if (!sdk_path) {
	//	ng->use_nvafx = false;
	//} else {
	//	char *file = "\\models\\denoiser_48k.trtpkg";
	//	ng->sdk_path = "C:\\Program Files\\NVIDIA Corporation\\NVIDIA Audio Effects SDK";
	//	ng->model = strcat(sdk_path, file);
	//}
	char *file = "\\models\\denoiser_48k.trtpkg";
	//next line is not used in this branch
	ng->sdk_path =
		"C:\\Program Files\\NVIDIA Corporation\\NVIDIA Audio Effects SDK";
	// change this path: it's hardcoded to the denoiser model on my drive (pkv)
	ng->model =
		"E:\\Downloads\\visual-studio\\Audio_Effects_SDK_v0.5\\bin\\models\\turing\\denoiser_48k.trtpkg";
	// For some reason, I fail to load the model from nvafx redist folder; maybe that's on account of the spaces ?
	//	ng->model = "\"C:\\Program Files\\NVIDIA Corporation\\NVIDIA\ Audio\ Effects\ SDK\\models\\denoiser_48k.trtpkg\""; 
...
static inline void process_nvafx(struct noise_suppress_data *ng)
{
	...
	for (int i = 0; i < MAX_PREPROC_CHANNELS; i++) {
		if (NvAFX_SetFloat(
			    ng->handle[i], NVAFX_PARAM_DENOISER_INTENSITY_RATIO,
			    ng->intensity_ratio) != NVAFX_STATUS_SUCCESS) {
			do_log(LOG_ERROR,
	...
	if (ng->nvafx_resampler) {
		...
		uint32_t out_frames;
		uint64_t ts_offset;
		audio_resampler_resample(ng->nvafx_resampler,
					 (uint8_t **)output, &out_frames,
					 &ts_offset,
					 (const uint8_t **)ng->copy_buffers,
					 (uint32_t)ng->frames);
		...
		for (size_t i = 0; i < ng->channels; i++) {
			for (ssize_t j = 0,
				     k = (ssize_t)out_frames - NVAFX_FRAME_SIZE;
	...
	for (size_t i = 0; i < ng->channels; i++) {
		NvAFX_Status err =
			NvAFX_Run(ng->handle[i], &ng->nvafx_segment_buffers[i],
...
static obs_properties_t *noise_suppress_properties(void *data)
{
	...
	obs_property_t *nvafx_slider = obs_properties_add_float_slider(
		...
		0.01f);
	obs_property_float_set_suffix(nvafx_slider, TEXT_NVAFX_INTENSITY);
#endif
	return ppts;
}
