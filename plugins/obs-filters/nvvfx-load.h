...
static HMODULE nv_videofx[2] = {NULL, NULL};
static HMODULE nv_cvimage[2] = {NULL, NULL};
static HMODULE nv_cudart[2] = {NULL, NULL};
...
static inline void release_nv_vfx(int fx)
{
...
static inline bool load_nv_vfx_libs(int fx)
{
	char fullPath[MAX_PATH];
	nvvfx_get_sdk_path(fullPath, MAX_PATH);
	SetDllDirectoryA(fullPath);
	nv_videofx[fx] = LoadLibrary(L"NVVideoEffects.dll");
	nv_cvimage[fx] = LoadLibrary(L"NVCVImage.dll");
	nv_cudart[fx] = LoadLibrary(L"cudart64_110.dll");
	SetDllDirectoryA(NULL);
	return !!nv_videofx[fx] && !!nv_cvimage[fx] && !!nv_cudart[fx];
}
