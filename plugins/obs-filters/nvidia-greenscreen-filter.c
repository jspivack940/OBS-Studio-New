...
nv_greenscreen_filter_video(void *data, struct obs_source_frame *frame)
{
	...
	/* 2. Pass obs video data to an NvCVImage */
	...
	/* 4. Convert video data to BGR on GPU */
...
static void nv_greenscreen_filter_tick(void *data, float seconds)
{
	struct nv_greenscreen_data *filter = (struct nv_greenscreen_data *)data;
	filter->tick_flag = true;
}
...
static void nv_greenscreen_filter_render(void *data, gs_effect_t *effect)
{
	...
	/* Skip if processing of a frame hasn't yet started or if there's no new frame. */
	if (width == 0 || !filter->A_dst_img || filter->tick_flag == false) {
