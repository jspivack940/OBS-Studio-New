...
static void nvafx_get_path(wchar_t *path)
{
	char value[MAX_PATH];
	...
	wcsncpy(path, (LPCWSTR)pvData, MAX_PATH);
}
...
static char *nvafx_get_sdk_path()
{
	char value[MAX_PATH];
	char buffer[MAX_PATH];
	PVOID pvData = value;
...
static bool load_lib(void)
{
	...
	nvafx_get_path(path);
	//mbstowcs(path, libpath, MAX_PATH);
