...
bool WHIPOutput::Setup()
{
	...
	rtcSetStateChangeCallback(peer_connection, [](int, rtcState state,
		...
		case RTC_CONNECTING:
			...
			whipOutput->start_time = os_gettime_ns();
			break;
		case RTC_CONNECTED:
			do_log(LOG_INFO, whipOutput->output,
			...
			whipOutput->connect_time_ms =
				(int)((os_gettime_ns() -
				       whipOutput->start_time) /
				      1000000.0);
			do_log(LOG_INFO, whipOutput->output,
			       "Connect time: %lf sec",
			       (double)whipOutput->connect_time_ms / 1000.0);
			whipOutput->running = true;
			break;
...
void register_whip_output()
{
	...
	// TODO: dropped frames, congestion
	info.get_dropped_frames = [](void *) -> int { return 0; };
	info.get_congestion = [](void *) -> float { return 0.0f; };
