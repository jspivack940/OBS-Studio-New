...
VolControl::VolControl(OBSSource source_, bool *mutePtr, bool showConfig,
		bool vertical, bool showMon, bool showTracks, int trackIndex)
		...
		vertical      (vertical)
{
	...
	if (trackIndex >= 0) {
		...
		std::string trackNum = "Track" + std::to_string(trackIndex + 1)
				+ "Name";
	...
	if (trackIndex >= 0) {
		...
		bool onRec = false;
		std::string RecMode = config_get_string(main->Config(),
				"AdvOut", "RecType");
		bool isStandard = RecMode.compare("Standard") == 0;
		if (isStandard) {
	...
	if (trackIndex < 0) {
		...
		if (source != nullptr)
			signal_handler_connect(
					obs_source_get_signal_handler(source),
					"audio_mixers", OBSSourceMixersChanged,
					this);
	}
...
VolControl::~VolControl()
{
	...
	if (source != nullptr) {
		...
		signal_handler_disconnect(obs_source_get_signal_handler(source),
				"monitoring_type", OBSSourceMonitoringChanged, this);
	}
