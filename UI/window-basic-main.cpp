...
void OBSBasic::on_actionAddCefWidget_triggered()
{
#ifdef BROWSER_AVAILABLE
	std::string path;
	bool success = CefWidgetDialog::AskForUrl(this, QTStr("Basic.Url.Title"),
	...
	for (QPointer<QWidget> widget : customCefWidgets) {
		std::string url = widget.data()->windowTitle().toStdString();
		if (url == path) {
			found = widget.data();
			break;
		}
	...
	QCefWidget *cefWidget = cef->create_widget(nullptr, path, panel_cookies);
	...
	widget->setWindowTitle(QString::fromStdString(path));
	...
	info->geometry = std::string(widget->saveGeometry().toBase64()
			.constData());
	info->url = path;
...
obs_data_array_t *OBSBasic::SaveBrowserPanels()
{
#ifdef BROWSER_AVAILABLE
	obs_data_array_t *savedBrowsers = obs_data_array_create();

	auto saveBrowser = [savedBrowsers](QCefWidget *cefWidget) {
		std::string url = cefWidget->windowTitle().toStdString();
		if (url.empty())
			return;
		obs_data_t *data = obs_data_create();
		...
		obs_data_set_string(data, "geometry",
				cefWidget->saveGeometry().toBase64()
					.constData());
...
void OBSBasic::Save(const char *file)
{
	...
	obs_data_array_t *savedBrowserPanels = SaveBrowserPanels();
...
void OBSBasic::LoadCefWidgets(obs_data_array_t *array)
{
	...
	for (size_t i = 0; i < num; i++) {
		...
		SavedCefWidgetInfo *info = new SavedCefWidgetInfo();
		info->geometry = std::string(obs_data_get_string(data,
				"geometry"));
		info->url = std::string(obs_data_get_string(data, "url"));
		savedCefWidgetsArray.emplace_back(info);

		/* Get previous geometry from last run */
		...
		widget->setWindowTitle(QString::fromStdString(info->url));
		widget->setVisible(true);
		...
		if (!WindowPositionValid(widget->normalGeometry())) {
			QRect rect = App()->desktop()->geometry();
